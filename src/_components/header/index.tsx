...
export function Header() {
  ...
  return (
    <div className=" bg-white h-20 w-full shadow-lg flex justify-between p-4 fixed ">
      ...
      {user && (
        <div className="flex py-4 gap-5  items-center">
          <div className="rounded-full bg-gray-300 px-3 py-2 flex gap-3 items-center">
          ...
          <button onClick={onClick} className="cursor-pointer">
            <Icon path={mdiInformationVariantCircleOutline} size={1.5} color={"#048BA8"}/>
          </button>
          <button onClick={()=> logout()} className="cursor-pointer">
            <Icon path={mdiPowerStandby} size={1.5} color={token.colorPrimary}/>
          </button>
        </div>
      )}
    </div>
